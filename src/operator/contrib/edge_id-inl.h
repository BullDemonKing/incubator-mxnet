...
struct edge_id_csr_forward {
  ...
  MSHADOW_XINLINE static void Map(int i, DType* out_data, const DType* in_data,
    ...
    const int target_row_id = static_cast<int>(u[i]);
    const int target_col_id = static_cast<int>(v[i]);
    out_data[i] = DType(-1);
    for (IType idx = in_indptr[target_row_id];
         idx < in_indptr[target_row_id + 1];
         ++idx) {
      if (in_indices[idx] == target_col_id) {
        out_data[i] = in_data[idx];
      }
    }
  }
};
